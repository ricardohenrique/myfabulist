                ...
                @foreach ($this->tree->folders as $navigationFolder)
                    ...
                    <div wire:key="folder-{{ $folder->id }}" wire:sort:item="{{ $folder->id }}" x-data="{ open: $persist(true).as('folder-open-{{ $folder->id }}') }">
                        <div class="group flex items-center gap-1 px-3 py-1.5">
                            {{-- Dedicated drag handle (R5) — keeps the
                                 collapse toggle, the "+" button and the
                                 dropdown clickable. Decorative only: the
                                 keyboard path is the Move up/down menu items
                                 below. --}}
                            <span
                                wire:sort:handle
                                aria-hidden="true"
                            ...
                            <button
                                ...
                                class="flex flex-1 items-center gap-2 text-sm font-medium text-zinc-500 dark:text-zinc-400"
                            >
                                <flux:icon.chevron-down x-bind:class="open ? 'rotate-0' : '-rotate-90'" class="size-4 shrink-0 transition-transform" />
